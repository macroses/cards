...
import { periodizationPrograms } from '@/data/periodizationPrograms'
...
<template>
  <ul class="programs-list">
    <ContainerUI
      v-for="program in periodizationPrograms"
      :key="program.path"
      tag="li"
      class="programs-list__item"
    >
      <h2 class="programs-list__item-title">
        {{ program.title }}
      </h2>
      <!--      <NuxtImg
        src="images/linear.svg"
        class="programs-list__item-img"
      /> -->
      <div class="programs-list__item-svg-wrap">
        <svg class="programs-list__item-svg" width="280" height="131" viewBox="0 0 280 131" fill="none" xmlns="http://www.w3.org/2000/svg" style="position: relative; overflow: visible;">
          ...
          <path id="intensity" d="M0.75 108.983C164 111 166.5 -61.5 246 51.5" stroke="#FF4000" stroke-opacity="0.5" stroke-width="2" stroke-dasharray="8 2" fill="none" />
          ...
          <!-- Анимированные круги -->
          <circle class="skill-dot" r="3" fill="red" />
          <circle class="intensity-dot" r="3" fill="red" />
          <circle class="volume-dot" r="3" fill="red" />
        </svg>
      </div>

      <ul class="programs-list__thesis">
        <li
          v-for="(thesis, index) in program.theses"
          :key="index"
          class="programs-list__thesis-item"
        >
          {{ thesis }}
        </li>
      </ul>
